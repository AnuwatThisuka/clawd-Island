#!/usr/bin/env bash
# Build "Clawd Island.app" and a shareable zip from the SwiftPM package.
#
...
#   • Set up once:  xcrun notarytool store-credentials "clawd-island-notary" \
...
BIN="$ROOT/.build/release/ClawdIsland"
...
install_name_tool -add_rpath "@executable_path/../Frameworks" "$APPDIR/Contents/MacOS/ClawdIsland" 2>/dev/null || true
...
ditto -c -k --sequesterRsrc --keepParent "$APPDIR" "$DIST/ClawdIsland.zip"
