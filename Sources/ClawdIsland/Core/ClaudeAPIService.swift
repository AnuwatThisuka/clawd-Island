...
actor ClaudeAPIService {
    ...
    private func parse(_ obj: [String: Any], source: String) -> ClaudeLimits {
        func node(_ key: String) -> (Double?, Date?) {
            ...
            let reset = resetStr.flatMap(ISO8601.date(from:))
    ...
    private func readCookies(from source: Source) -> [String: String]? {
        ...
        let base = fm.temporaryDirectory
            .appendingPathComponent("clawd-\(abs(source.path.path.hashValue))-\(getpid()).sqlite")
    ...
    private func readChromium(_ db: OpaquePointer, service: String) -> [String: String]? {
        ...
        // Exact host or a real subdomain only. A broad LIKE '%claude.ai%' would also match
        // look-alike domains (e.g. claude.ai.evil.com, notclaude.ai), pulling in cookies that
        // are not actually claude.ai's; scope it to claude.ai and *.claude.ai.
        let sql = "SELECT name, encrypted_value FROM cookies WHERE host_key = 'claude.ai' OR host_key LIKE '%.claude.ai'"
    ...
    private func readFirefox(_ db: OpaquePointer) -> [String: String]? {
        var stmt: OpaquePointer?
        // Exact host or a real subdomain only — same reasoning as readChromium: keep look-alike
        // domains (claude.ai.evil.com, notclaude.ai) out of the result set.
        let sql = "SELECT name, value FROM moz_cookies WHERE host = 'claude.ai' OR host LIKE '%.claude.ai'"
        guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK else { return nil }
        defer { sqlite3_finalize(stmt) }
        var out: [String: String] = [:]
        while sqlite3_step(stmt) == SQLITE_ROW {
    ...
    /// `internal` (not `private`) only so the tests can drive it via `@testable import`;
    /// it is not part of any public API.
    func decrypt(_ enc: Data, key: Data) -> String? {
